...
export class SpotifyService {
  ...
  constructor(private http: HttpClient) {
    this.temporizador = interval(1000).pipe(startWith(0)).subscribe(segundos=>{
  ...
  getToken() {
    const pedirTokenCadaSegundos:number = 20; //Indica cada cuanto tiempo pedir un token, se pide cada 55 minutos cuando el token expira
    ...
    .toPromise().then( (token:any) => {
      if (this.token==='' || this.valorTemporizador >= this.segundos){ // Si el temporizador supero el tiempo limite de vida del token, se pide uno nuevo
        ...
        console.log(this.token);}
     else{/* console.log("Token activo. No hay que pedir token"); */}
    }, (err: any) => {
     console.log(err);
    });
  }
  ...
  return this.http.get('https://api.spotify.com/v1/browse/new-releases?limit=24', { headers }).toPromise();
}
  ...
  // Obtener Artistas
   async getArtistas( contenidoBusqueda: string ){
    ...
    return this.http.get(`https://api.spotify.com/v1/search?q=${ contenidoBusqueda }&type=artist&limit=16`, { headers }).toPromise();
  }
  ...
  // Obtener Canciones
   async getTracks( contenidoBusqueda: string ){
    ...
    /* Solo cambia la parte que va desde q= hasta &type */
    return this.http.get(`https://api.spotify.com/v1/search?q=${ contenidoBusqueda }&type=track&limit=12`, { headers }).toPromise();
  }
}
